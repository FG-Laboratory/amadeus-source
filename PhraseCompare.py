...
class dialline():
	...
	def __str__(self):
		return json.dumps({'orig':self.orig,'words':self.words})
	def __repr__(self):
...
#	print(k)
	return math.exp(k*1.54)#Характеризует, как долго бот будет помнить дилог, из-за моих кривых ручонок требуем роста функции, а не затухания
...
#		logD(str(weights))
		for pos in range(len(self.diallines)-dial_l+1):
			...
			for i in range(dial_l-2,-1,-1):#С последней фразы начинаем поиск
				...
				try:#Патч бреда
					weakeffect=len(self.diallines[pos+dial_l].words)
					if(weakeffect>7):
						summ-=weightFunction(-2)*(weakeffect-7)/3
				except Exception as e:
					logD("getAnswerError: "+str(e))
			if(summ>=maxval):
