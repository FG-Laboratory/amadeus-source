		...
		try:
			...
			try:
				...
				try:
					...
					sendMsg(str(peer),"Я Макисэ Курису. Рада познакомиться ^-^")
				except:
					pass
			isVoice=False
			try:
			...
			if(txt.count("/")>0):
				print(i)
			if(txt.count("/help")>0):
				sendMsg(str(peer),"Бот для развлечения, практического смысла не имеет или почти не имеет. Возможно, когда-нибудь научится имитировать Курису Макисэ")
				continue
	...
	if(random.random()<0.000545):
		try:
			if(random.random()>0.6):
				...
				sendSticker(random.choice(dm.dialogids).replace("_tg",""))
		except Exception as e:
			print(e)
	time.sleep(2)
