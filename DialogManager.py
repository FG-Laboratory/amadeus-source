...
import knowledger
dialAdapters.append(knowledger.DialogClass())
...
def ifAllEq(arr):
	for i in range(len(arr)-1):
		if(arr[i].replace(" ","")!=arr[i+1].replace(" ","")):return False
	return True

class Dialog():
	privScore=0.1#Какая уверенность для ответа в личной беседе
	pubScore=0.77#Какая уверенность для ответа в беседе с несколькими участниками
	lastCall=-1
	...
	def sendAnswer(self,txt,isClear=False):#Если isClear=True, то сообщение отправляется сразу и без проверок
		...
		if(self.__lastsent==txt):
			if(utils.getSemanticLoad(txt)<0.8):
				self.isAnswered=False
				logD("Null Answer")
				return
		self.__lastsent=txt
...
#			self.dialsit.append(txt)
	...
	def getAnswer(self,txt,pictxt="",isPrivate=True):#isPrivate решает, молчать в случае сомнений или отвечать
		...
		if(self.isWasNotPrivate):#Добавить текущую строку
			...
			if(len(self.localdial.diallines)>5000):self.localdial.diallines=self.localdial.diallines[-5000:]
